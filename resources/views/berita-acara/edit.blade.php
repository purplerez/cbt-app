<x-app-layout>
        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route(request()->route()->getPrefix() . '.berita-acara.update', $beritaAcara) }}" id="beritaAcaraForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select name="exam_type_id" id="exam_type_id" required 
                                    ...
                                    @foreach($examTypes as $type)
                                        <option value="{{ $type->id }}" {{ (old('exam_type_id') ?? $beritaAcara->exam_type_id) == $type->id ? 'selected' : '' }}>
                                            {{ $type->title }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="school_id" id="school_id" required 
                                    ...
                                    @foreach($schools as $school)
                                        <option value="{{ $school->id }}" 
                                                {{ (old('school_id') ?? $beritaAcara->school_id) == $school->id ? 'selected' : '' }}>
                                            {{ $school->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="tanggal_pelaksanaan" id="tanggal_pelaksanaan" 
                                       value="{{ old('tanggal_pelaksanaan', $beritaAcara->tanggal_pelaksanaan->format('Y-m-d')) }}" required
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                            ...
                            <div>
                                ...
                                <input type="number" name="jumlah_peserta_terdaftar" id="jumlah_peserta_terdaftar" 
                                       value="{{ old('jumlah_peserta_terdaftar', $beritaAcara->jumlah_peserta_terdaftar) }}" required min="0"
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div>
                                ...
                                <input type="number" name="jumlah_peserta_hadir" id="jumlah_peserta_hadir" 
                                       value="{{ old('jumlah_peserta_hadir', $beritaAcara->jumlah_peserta_hadir) }}" required min="0"
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <div>
                                ...
                                <input type="number" name="jumlah_peserta_tidak_hadir" id="jumlah_peserta_tidak_hadir" 
                                       value="{{ old('jumlah_peserta_tidak_hadir', $beritaAcara->jumlah_peserta_tidak_hadir) }}" required min="0"
                                       class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                            ...
                            <div>
                                ...
                                <select name="kondisi_pelaksanaan" id="kondisi_pelaksanaan" required
                                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <option value="lancar" {{ (old('kondisi_pelaksanaan') ?? $beritaAcara->kondisi_pelaksanaan) == 'lancar' ? 'selected' : '' }}>Lancar</option>
                                    <option value="ada_kendala" {{ (old('kondisi_pelaksanaan') ?? $beritaAcara->kondisi_pelaksanaan) == 'ada_kendala' ? 'selected' : '' }}>Ada Kendala</option>
                                    <option value="terganggu" {{ (old('kondisi_pelaksanaan') ?? $beritaAcara->kondisi_pelaksanaan) == 'terganggu' ? 'selected' : '' }}>Terganggu</option>
                                </select>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="kendala" id="kendala" rows="3"
                                      class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('kendala', $beritaAcara->kendala) }}</textarea>
                                    ...
                                    <div>
                                        <label class="inline-flex items-center">
                                            <input type="checkbox" name="pengawas[]" value="{{ $teacher->id }}" 
                                                   {{ in_array($teacher->id, old('pengawas', $beritaAcara->pengawas ?? [])) ? 'checked' : '' }}
                        ...
                        <div>
                            ...
                            <textarea name="catatan_khusus" id="catatan_khusus" rows="4"
                                      class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('catatan_khusus', $beritaAcara->catatan_khusus) }}</textarea>
    ...
    <script>
        ...
        document.getElementById('autoFillBtn').addEventListener('click', async function() {
            ...
            try {
                const response = await fetch('{{ route(request()->route()->getPrefix() . ".berita-acara.autofill") }}', {
                    method: 'POST',
                    headers: {
