<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3 text-sm text-gray-500">
                ...
                <button onclick="refreshAll()"
                    class="flex items-center gap-1 px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs hover:bg-indigo-700 transition">
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                <div
                    class="p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div class="flex items-center gap-3">
                        <select id="exam-select" onchange="loadMonitor()"
                            class="text-sm border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-gray-50 text-gray-900">
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Sisa Waktu
                                </th>
                        ...
                        <tbody id="monitor-body" class="bg-white divide-y divide-gray-50">
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-gray-400 text-sm">Pilih ujian
                                    untuk memulai monitoring</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Kelas</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Rata-rata
                                </th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Peserta
                                </th>
                            </tr>
                        </thead>
                        <tbody id="scores-body" class="bg-white divide-y divide-gray-50">
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center text-gray-400 text-sm">Memuat data
                                    nilai...</td>
                            </tr>
                        </tbody>
                    </table>
    ...
    <div id="reset-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black/40">
        <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
            <h4 class="text-base font-semibold text-gray-800 mb-2">Unlock Akun Siswa</h4>
            <p class="text-sm text-gray-600 mb-1">Yakin ingin unlock akun siswa yang terkunci force exit:</p>
            <p class="text-sm font-semibold text-gray-900 mb-1" id="modal-student-name">—</p>
            <p class="text-xs text-gray-500 mb-4">Siswa akan dapat melanjutkan ujian mereka. Status siswa akan diubah
                menjadi siap ujian (is_active = 1, is_logout = 0).</p>
            <div class="flex gap-3 justify-end">
                <button onclick="closeResetModal()"
                    class="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 transition">Batal</button>
                <button onclick="confirmReset()" id="confirm-reset-btn"
                    class="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Unlock
                    Akun</button>
            </div>
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            ...
            async function fetchStats() {
                try {
                    const r = await fetch('/api/kepala/dashboard/stats', {
                        headers: HEADERS
                    });
                    ...
                    try {
                        ...
                        console.error('Stats Error:', text.substring(0, 100));
                        return;
                    }
                    if (d && d.error) return;
                    ...
                    document.getElementById('last-updated').textContent = 'Update: ' + new Date().toLocaleTimeString(
                        'id-ID');
                } catch (e) {}
            }
            ...
            async function fetchActiveExams() {
                try {
                    const r = await fetch('/api/kepala/monitor/active-exams', {
                        headers: HEADERS
                    });
                    const text = await r.text();
                    let d;
                    try {
                        ...
                        console.error('Active Exams Error:', text.substring(0, 100));
                        return;
                    }
            ...
            function loadMonitor() {
                ...
                fetchMonitor(id);
                monitorInterval = setInterval(() => fetchMonitor(id), 30000);
            }

            async function fetchMonitor(examId) {
                try {
                    document.getElementById('monitor-body').innerHTML =
                        '<tr><td colspan="6" class="px-4 py-6 text-center text-gray-400 text-xs">Memuat data...</td></tr>';
                    const r = await fetch(`/api/kepala/monitor/exam/${examId}/participants`, {
                        headers: HEADERS
                    });
                    const text = await r.text();
                    let res;
                    try {
                        ...
                        throw new Error('Data server tidak valid (Bukan JSON).');
                    }
                    ...
                    if (!res.data || !res.data.length) {
                        document.getElementById('monitor-body').innerHTML =
                            '<tr><td colspan="6" class="px-4 py-10 text-center text-gray-400 text-sm">Tidak ada peserta terdaftar.</td></tr>';
                        return;
                    }
                    document.getElementById('monitor-body').innerHTML = res.data.map(p => {
                        ...
                        const timeD = p.status === 'progress' ?
                            `<span class="font-mono text-sm text-orange-600" data-seconds="${p.time_remaining}">${fmtTime(p.time_remaining)}</span>` :
                ...
                <td class="px-4 py-3">${actions}</td>
                    ...
                    document.getElementById('monitor-body').innerHTML =
                        `<tr><td colspan="6" class="px-4 py-6 text-center text-red-400 text-sm">${e.message}</td></tr>`;
                }
            }
            ...
            async function fetchRecentScores() {
                try {
                    const r = await fetch('/api/kepala/dashboard/recent-scores', {
                        headers: HEADERS
                    });
                    const text = await r.text();
                    let d;
                    try {
                        ...
                        console.error('Scores error:', text.substring(0, 100));
                        return;
                    }
                    ...
                    if (!list.length) {
                        tbody.innerHTML =
                            '<tr><td colspan="4" class="px-4 py-8 text-center text-gray-400 text-sm">Belum ada ujian yang selesai.</td></tr>';
                        return;
                    }
            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="px-4 py-3 text-sm text-gray-600">${s.participant_count} siswa</td>
                    ...
                    document.getElementById('scores-body').innerHTML =
                        '<tr><td colspan="4" class="px-4 py-6 text-center text-gray-400 text-sm">Gagal memuat nilai.</td></tr>';
                }
            }
            ...
            function startTick() {
                ...
                timerTick = setInterval(() => {
                    document.querySelectorAll('[data-seconds]').forEach(el => {
                        let s = Math.max(0, parseInt(el.dataset.seconds) - 1);
            ...
            async function confirmReset() {
                ...
                try {
                    const r = await fetch(`/kepala/exam-sessions/${currentNis}/reset`, {
                        method: 'POST',
                        headers: {
                            ...HEADERS,
                            'X-CSRF-TOKEN': CSRF,
            ...
            function toast(type, msg) {
                ...
                t.className =
                    `fixed bottom-4 right-4 z-50 px-4 py-3 text-white rounded-lg shadow-lg text-sm ${type==='success'?'bg-green-500':'bg-red-500'}`;
                t.textContent = msg;
